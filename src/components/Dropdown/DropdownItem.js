...
const DropdownItem = (props) => {
  ...
  const metrosHandler = (event) => {
    setMetros(event.target.value);
    props.onAltura(metros);
  };

  const centimetrosHandler = (event) => {
    setCentimetros(event.target.value);
  };

  return (
    <div className={styles["dropdown-item"]}>
      <select onChange={metrosHandler} placeholder="Mt">
      ...
      <select onChange={centimetrosHandler} placeholder="Cm">
